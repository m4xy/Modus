...
5. **Optimistic concurrency replaces row versioning.** Every mutation is conditional on the
...
6. **No multi-document transactions.** This is the storage-layer face of the one-aggregate
...
7. **Indexes are derived and disposable.** They live under a git-ignored `.modus/index/`,
...
8. **Recovery is explicit and never repairs data.** Startup sweeps orphan temp files,
...
9. **The ban is mechanical.** No `java.sql`, `javax.sql`, `jakarta.persistence`,
   `org.hibernate`, or `org.jooq` type may be referenced anywhere, and a database driver on
   any Gradle configuration fails the build. Enforced by ArchUnit and a Gradle
   dependency-verification rule (`00-constitution.md` §2, `10-architecture.md` §4.1).

10. **Documented scale limits.** `40-durability.md` §10 states, per dimension, where the
...
11. **The domain does not know.** `core-domain` sees repository ports and no file paths.
...
- **No ad-hoc query.** Every access path must be anticipated as a purpose-named repository
  ...
  *Mitigation:* the index layer (§7); the workload's queries are few and known.
...
- **Single-writer.** Horizontal scale-out of writes is not available without a superseding
  ADR. *Mitigation:* the workload is human-paced; §10's limits make the boundary explicit.
- **We implement durability ourselves.** `fsync` ordering, `rename` atomicity, `O_APPEND`
  semantics, torn-line recovery, and lock breaking are subtle, and a database vendor has
  spent decades on them. *Mitigation:* one narrow implementation surface (one writer class,
  one append path), plus a crash-consistency test suite that `SIGKILL`s the process at
  randomised points and asserts every acknowledged cursor survives.
...
| **Files now, database later behind the same ports** | Keeps the option open at low cost. | This *is* the position — see Decision §11. The ports make reversal a contained change. What we reject is adding the database *now*, before any measurement shows a need. §10's limits define what that measurement would have to show. |
