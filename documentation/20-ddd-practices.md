...
| 4.1.7 | Every event is appended to the durable event log before any handler runs. Handlers are idempotent — they may be replayed. **Enforcement gap:** there is no durable event log and nothing appends a domain event anywhere, so no handler in this repository has ever run behind one; `bean:0160` carries it, and §4.1.8's data loss is its direct consequence. |
| 4.1.8 | **A handler that throws is neither swallowed nor absorbed: it propagates, and delivery of that batch stops there.** Subscriptions after the failing one, and events after the failing one, are not delivered — delivery is all-or-a-prefix, never all-or-nothing. Every implementation of the dispatch port is held to this, and each one says so rather than restating why. Two things follow, and the second is the cost. A use case's caller sees the failure with the write already committed, which is correct: the write is durable and announcing it is what failed. And **the undelivered suffix is lost permanently** — nothing recorded that those events were due, so nothing can replay them. That is §4.1.7's missing log, not this rule: running every handler and reporting failures afterwards discards exactly the same events, and adds a summary no consumer reads. Recorded rather than resolved, because the fix is the log (`bean:0160`, `bean:0066`). |
