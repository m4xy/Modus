...
| 4.1.7 | Every event is appended to the durable event log before any handler runs. Handlers are idempotent — they may be replayed. **Enforcement gap:** there is no durable event log and nothing appends a domain event anywhere, so no handler in this repository has ever run behind one and no delivery is replayable. Two consequences hold until `bean:0160` closes it, and neither is a rule: an event a dispatcher does not deliver is lost with no record that it was due — §4.1.8's undelivered suffix included — and a dispatcher that instead ran every handler and reported the failures afterwards would lose exactly the same events. The fix is the log, not the delivery policy. |
| 4.1.8 | **A handler that throws is neither swallowed nor absorbed, and every implementation of the dispatch port states which delivery mode it is.** In **synchronous** delivery — `dispatch` returns only after its handlers have run — the exception propagates to the caller and delivery of that batch stops there: subscriptions after the failing one, and events after the failing one, are not delivered, so delivery is all-or-a-prefix rather than all-or-nothing. The caller sees the failure with the write already committed, which is correct — the write is durable and announcing it is what failed. An **asynchronous** implementation cannot propagate to a caller that has already returned and is bound by the first clause alone: it records the failure where a replay can find it (§4.1.7) and never drops it silently. Scoping this by mode rather than universally is deliberate — §4.1.7 effectively mandates a durable implementation, and a rule that bound one to propagate synchronously would bind it to behaviour it cannot exhibit. **Enforcement gap:** the mode declaration is prose in a KDoc and nothing parses it, so an implementation that declares nothing, or declares the mode it is not, fails no build. A marker interface or a sealed pair on the port would make it mechanical today, while there is one implementor to migrate. `bean:0163` carries it. |
