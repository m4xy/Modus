...
**One elision below was not an elision of anything.** Criterion 8 read
`docs-lint: OK [...] 0 failure(s)` for four review rounds. `tools/docs-lint.sh` prints
`docs-lint: $n_fail failure(s).` and exits 1, *or* it prints the `docs-lint: OK — …` counts
line; the failure branch is guarded by `n_fail > 0`. **No run of that script has ever
produced the string that stood there, and no run can** — not on a stale tree, not on any
tree. The `[...]` did not shorten an output, it joined two mutually exclusive branches, and
what it hid was the twelve counts the script's own comment calls the vacuity assertion.
That is worth recording rather than merely fixing: a stale transcript is falsified by
re-running the command, and this one is not — it reads identically on the day it is written
and on every day after, because there is no day on which it was right.
...
| 7 | Sonnet 5's constant matches `tools/cost_lib.py`'s `BASE_RATES_UPM` for the same model id — the authority this seam is accountable to — and the date on which that figure stops matching `doc:60-cost-model#price-book` is stated rather than left to be found | `cmd`: `grep -n "claude-sonnet-5" backoffice/src/agent/transport.ts tools/cost_lib.py` -> `'claude-sonnet-5': { input: 2_000_000, output: 10_000_000 },` and `"claude-sonnet-5": (2_000_000, 10_000_000),`. Both halves of the seam carry the same figure, which is what the criterion asserts and the whole of what it asserts. **It does not claim the figure is the rate in force, and an earlier revision of this cell did.** The figure is the introductory $2/$10: it held through 2026-08-31, the standard $3/$15 applies from 2026-09-01, and from that date the constant reads 33% low against `doc:60-cost-model#price-book`. It is not moved here. `cost_lib.py`'s own comment records that every run in the replayed corpus predates the lapse, so the constant is right for the corpus it prices and wrong for a later run — two true statements only a price book with effective dates can hold at once, and `domains/<domainId>/cost/price-book.md` does not exist yet. Moving this half alone would price the corpus wrongly and re-create the 50% seam disagreement this bean was raised to fix. The earlier `$3/$15` was 50% high on every day the introductory rate held and is correct again from 2026-09-01. `bean:0090` instance 1 and its "dated sibling" section carry the shape and the open criterion |
| 8 | The gate is green | `command`: `./gradlew backofficeTypecheck backofficeLint backofficeFormatCheck` -> `BUILD SUCCESSFUL`. `command`: `./gradlew docsLint` -> `docs-lint: OK — 19 documents, 109 anchors, 1389 references, 95 beans, 37 graph edges, 43 selectable, 95 bean ids, 6 introduced, 89 on origin/main, 1 closing transitions, 3 criteria checked, 0 unnumbered.` — captured whole. **The earlier cell read `docs-lint: OK [...] 0 failure(s)`, which no run can print.** `tools/docs-lint.sh` either prints `docs-lint: $n_fail failure(s).` and exits 1, or prints the `OK —` counts line; the branches are mutually exclusive and the failure branch is guarded by `n_fail > 0`, so `0 failure(s)` is unreachable. The `[...]` concealed a splice of the two, and it concealed precisely the twelve counts the script's own comment calls the vacuity assertion that `doc:00-constitution#observed-failing` asks for — an elision that removed the only part of the line carrying information. The counts move with any edit to any bean or document, this cell included, so the line above is the run on this branch final tree and a later tree gives different numbers under the same `OK`. `test-run`: `./gradlew e2eTest` -> `37 passed`, on three of four consecutive runs; the fourth is read below |
...
- The evidence deliberately carries **three** observations, not one: the detector fires on a
  ...
  **And §9.1 still does not carry it.** Its MUST bullets require only the planted violation,
  ...
  `bean:0105` is closed as superseded and carries the recaptured sweep and the instance;
  `bean:0112` owns the general defect that the sweep establishing the original gap could not
  see `doc:50`. Collapsing this bullet into a bare citation of §2.2 is owned by neither and
  is not done here — it is named as a loose end in `bean:0090`'s criteria.
