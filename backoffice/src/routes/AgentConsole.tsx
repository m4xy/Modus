                        ...
                        {session.blocks.length === 0 ? (
                            ...
                            session.blocks.map((block) => (
                                <div className={styles.block} key={block.id}>
                                    ...
                                    <div>
                                        {block.kind === 'prompt' && (
                                            <p className={styles.promptText}>{block.text}</p>
                                        )}
                                        {block.kind === 'assistant' && (
                                        ...
                                        {block.kind === 'notice' && (
                                            <p className={styles.notice}>{block.text}</p>
                                        )}
                                    </div>
                                </div>
                            ))
                    ...
                    <div className={styles.composer}>
                        ...
                        <div className={styles.composerRow}>
                            ...
                            <span className={styles.hint}>
                                {canRun
                                    ? 'Runs are billed to this domain'
                                    : 'You cannot start runs in this domain'}
                            </span>
                                ...
                                <tbody>
                                    {(runs.data ?? []).map((item) => (
                                        <tr key={item.id}>
                                            ...
                                            <Td numeric mono>
                                                {formatTokens(item.tokensIn)} /{' '}
                                                {formatTokens(item.tokensOut)}
                                            </Td>
